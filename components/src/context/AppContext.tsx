...
import { AppView, Event, Settings, User, Category, Comment, Notification, ActivityLog, Toast } from '../types';
import { useAuth } from './AuthContext';

// ... (Translations object remains the same as your provided file)
const translations = {
  en: {
    ...
    'guide.section8.title': '7. Activity Log (for Admins)',
    ...
    'share.copied': 'Copied!',
    'toast.genericSuccess': 'Action successful!',
  ...
  ja: {
    ...
    'guide.section8.title': '7. アクティビティログ（管理者向け）',
    ...
    'toast.genericSuccess': '操作に成功しました！',
    'toast.genericError': 'エラーが発生しました。',
...
interface AppContextType {
  ...
  t: (key: string) => string;
  addEvent: (eventData: Omit<Event, 'id' | 'authorId'>) => Promise<void>;
  updateEvent: (event: Event) => Promise<void>;
  deleteEvent: (eventId: string, eventName: string) => Promise<void>;
  updateRecurringEvent: (event: Event, mode: 'single' | 'all') => Promise<void>;
  deleteRecurringEvent: (event: Event, mode: 'single' | 'all') => Promise<void>;
  ...
  addComment: (commentData: Omit<Comment, 'id' | 'createdAt' | 'authorId'>) => Promise<void>;
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const collectionsToSubscribe: (keyof AppState)[] = ['users', 'categories', 'events', 'comments', 'notifications', 'activityLog'];
        
        const unsubscribers = collectionsToSubscribe.map(colName => 
            onSnapshot(collection(db, colName as string), (snapshot) => {
        ...
        const unsubSettings = onSnapshot(doc(db, 'settings', 'main'), (doc) => {
            if (doc.exists()) {
                const settingsData = convertTimestamps(doc.data()) as Settings;
                setState(prevState => ({ ...prevState, settings: settingsData }));
                document.documentElement.lang = settingsData.language;
    ...
    // Set current user data from users list
    useEffect(() => {
        if (authUser && state.users) {
            const currentUserData = state.users.find(u => u.uid === authUser.uid) || null;
            setState(prevState => ({ ...prevState, currentUser: currentUserData }));
        }
    ...
    const addToast = (toast: Omit<Toast, 'id'>) => {
        ...
        setState(prevState => ({ ...prevState, toasts: [...prevState.toasts, { ...toast, id }] }));
    };
    ...
    const createLog = async (action: string, details: { [key: string]: any }) => {
    ...
    // CRUD functions... (addEvent, updateEvent, etc.)
    const addEvent = async (eventData: Omit<Event, 'id' | 'authorId'>) => {
        if (!state.currentUser) return;
        const newEvent = { ...eventData, authorId: state.currentUser.uid };
        const docRef = await addDoc(collection(db, 'events'), newEvent);
        await createLog('CREATE_EVENT', { eventName: newEvent.title });
        
        // Add notifications
        if (state.settings?.enableNotifications && state.users) {
            ...
            state.users.forEach(user => {
                if (user.uid !== state.currentUser?.uid) {
                    ...
                    const newNotif: Omit<Notification, 'id'> = {
                        userId: user.uid,
                        message: { type: 'new_event', authorName: state.currentUser?.name || 'User', eventName: newEvent.title },
                        eventId: docRef.id,
                        isRead: false,
                        createdAt: new Date(),
                    };
        ...
        addToast({ message: t('toast.genericSuccess'), type: 'success' });
    };
    ...
    const updateEvent = async (event: Event) => {
        ...
        await createLog('UPDATE_EVENT', { eventName: event.title });
        addToast({ message: t('toast.genericSuccess'), type: 'success' });
    };

    const deleteEvent = async (eventId: string, eventName: string) => {
        await deleteDoc(doc(db, 'events', eventId));
        await createLog('DELETE_EVENT', { eventName });
        addToast({ message: 'Event deleted.', type: 'info' });
    };
    
    const updateRecurringEvent = async (event: Event, mode: 'single' | 'all') => {
        const templateId = event.recurringEventId || event.id;
        if (mode === 'all') {
            const { id, ...templateData } = { ...event, recurringEventId: undefined, isRecurring: true };
            delete (templateData as any).originalStart;
            await updateDoc(doc(db, 'events', templateId), templateData);
        } else { // mode === 'single'
            const templateRef = doc(db, 'events', templateId);
            const exceptionDate = toISOYMD(new Date(event.originalStart || event.start));
            const currentTemplate = state.events?.find(e => e.id === templateId);
            const newExceptions = [...(currentTemplate?.exceptionDates || []), exceptionDate];
            await updateDoc(templateRef, { exceptionDates: newExceptions });

            const { id, ...newEventData } = { ...event, isRecurring: false, recurringEventId: undefined, originalStart: undefined };
    ...
    const deleteRecurringEvent = async (event: Event, mode: 'single' | 'all') => {
        const templateId = event.recurringEventId || event.id;
        if (mode === 'all') {
            await deleteDoc(doc(db, 'events', templateId));
        } else { // mode === 'single'
            const templateRef = doc(db, 'events', templateId);
            const exceptionDate = toISOYMD(new Date(event.originalStart || event.start));
            const currentTemplate = state.events?.find(e => e.id === templateId);
        ...
        await createLog('DELETE_EVENT', { eventName: event.title });
        addToast({ message: 'Event removed.', type: 'info' });
    };
    
    const updateSettings = async (settings: Partial<Settings>) => {
        const settingsRef = doc(db, 'settings', 'main');
        await updateDoc(settingsRef, settings);
        await createLog('UPDATE_SETTINGS', {});
        addToast({ message: t('toast.genericSuccess'), type: 'success' });
    };
    ...
    const toggleFavorite = async (eventId: string) => {
        if (!state.currentUser) return;
        const newFavorites = (state.currentUser.favorites || []).includes(eventId)
            ? (state.currentUser.favorites || []).filter(id => id !== eventId)
            : [...(state.currentUser.favorites || []), eventId];
    ...
    const updateUser = async (user: User) => {
        ...
        await createLog('UPDATE_USER', { userName: user.name });
        addToast({ message: t('toast.genericSuccess'), type: 'success' });
    };
    ...
    const deleteUser = async (user: User) => {
        ...
        if (user.uid === state.currentUser.uid) {
            addToast({ message: t('toast.deleteSelf'), type: 'error' });
            return;
        }
        const adminCount = state.users.filter(u => u.permissions.canManageUsers).length;
        if (user.permissions.canManageUsers && adminCount <= 1) {
            addToast({ message: t('toast.lastAdmin'), type: 'error' });
            return;
        }
        await deleteDoc(doc(db, 'users', user.uid));
        await createLog('DELETE_USER', { userName: user.name });
        addToast({ message: 'User deleted.', type: 'info' });
    };

    const setCategoryFilter = (filter: string | null) => {
        setState(prevState => ({ ...prevState, activeCategoryFilter: filter }));
    };

    const toggleFavoritesFilter = () => {
        setState(prevState => ({ ...prevState, isFavoritesFilterActive: !prevState.isFavoritesFilterActive }));
    };
    
    const addComment = async (commentData: Omit<Comment, 'id' | 'createdAt' | 'authorId'>) => {
        if (!state.currentUser || !state.settings?.enableComments) return;
        const newComment = { ...commentData, authorId: state.currentUser.uid, createdAt: new Date() };
        await addDoc(collection(db, 'comments'), newComment);
        
        if (state.settings.enableNotifications && state.users && state.events) {
            const event = state.events.find(e => e.id === newComment.eventId || e.recurringEventId === newComment.eventId);
            if (!event) return;

            const commenter = state.users.find(u => u.uid === newComment.authorId);
            ...
            const batch = writeBatch(db);
            Array.from(usersToNotifyIds).forEach(userId => {
                const notifRef = doc(collection(db, 'notifications'));
                const newNotif: Omit<Notification, 'id'> = {
                    userId: userId,
                    message: { type: 'new_comment', commenterName: commenter?.name || 'User', eventName: event.title },
                    eventId: event.id,
                    isRead: false,
                    createdAt: new Date(),
                };
    ...
    const addCategory = async (categoryData: Omit<Category, 'id'>) => {
        ...
        await createLog('CREATE_CATEGORY', { categoryName: categoryData.name });
        addToast({ message: t('toast.genericSuccess'), type: 'success' });
    };
    ...
    const updateCategory = async (category: Category) => {
        ...
        await updateDoc(catRef, catData);
        await createLog('UPDATE_CATEGORY', { categoryName: category.name });
        addToast({ message: t('toast.genericSuccess'), type: 'success' });
    };
    
    const deleteCategory = async (category: Category) => {
        if (state.events?.some(e => e.categoryId === category.id)) {
            addToast({ message: t('toast.categoryInUse'), type: 'error' });
            return;
        }
        await deleteDoc(doc(db, 'categories', category.id));
        await createLog('DELETE_CATEGORY', { categoryName: category.name });
    ...
    const markAllNotificationsAsRead = async () => {
        ...
        state.notifications.forEach(n => {
            if (n.userId === state.currentUser?.uid && !n.isRead) {
    ...
    const t = (key: string): string => {
        ...
        return translations[lang][langKey] || key;
    };
    ...
    const generatedEvents = useMemo(() => {
        ...
        return allEvents.filter(event => {
            return event.status === 'published' || event.authorId === state.currentUser?.uid;
        });
    }, [state.events, state.currentUser?.uid]);
