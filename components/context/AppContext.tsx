...
import { AppView, Event, Settings, User, Category, Language, Theme, Comment, Notification, NotificationMessage, ActivityLog, Toast } from '../types';
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const collections = ['users', 'categories', 'events', 'comments', 'notifications', 'activityLog'];
    ...
    useEffect(() => {
        if (authUser && state.users) {
            ...
            const userFavorites = currentUserData ? (currentUserData as any).favorites || [] : [];
            setState(prevState => ({ 
                ...prevState, 
    ...
    const updateRecurringEvent = async (event: Event, mode: 'single' | 'all') => {
        ...
        if (mode === 'all') {
            ...
            const updateData = { ...event, id: templateId, recurringEventId: undefined, isRecurring: true };
            delete updateData.originalStart;
            ...
            const newEventData = { ...event, isRecurring: false, recurringEventId: undefined, originalStart: undefined };
            delete (newEventData as any).id;
            await addDoc(collection(db, 'events'), newEventData);
        }
        await createLog('UPDATE_EVENT', { eventName: event.title });
        addToast({ message: 'Recurring event updated!', type: 'success' });
    };
    ...
    const deleteRecurringEvent = async (event: Event, mode: 'single' | 'all') => {
        ...
        addToast({ message: 'Recurring event removed.', type: 'info' });
    };
    ...
    const toggleFavorite = async (eventId: string) => {
        if (!state.currentUser) return;
        const newFavorites = state.favorites.includes(eventId)
    ...
    const deleteUser = async (user: User) => {
        ...
        // Note: This does not delete the Firebase Auth user. That requires a backend function.
    ...
    const t = (key: string): string => {
        ...
        return translations[lang][langKey] || langKey;
    };
