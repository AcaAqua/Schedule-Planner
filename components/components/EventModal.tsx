...
const EventModal: React.FC<EventModalProps> = ({ eventToEdit, onClose, setRecurringModalState, onShareRequest }) => {
  const { state, addEvent, updateEvent, deleteEvent, toggleFavorite, addComment, deleteComment } = useAppContext();
  const { t } = useTranslation();
  
  if (!state.categories || !state.currentUser || !state.users || !state.comments) return null;
  ...
  const author = eventToEdit ? users.find(u => u.uid === eventToEdit.authorId) : null;
  const isFavorite = eventToEdit ? favorites.includes(eventToEdit.id) : false;
  
  const canEdit = !eventToEdit || currentUser.role === 'admin' || currentUser.uid === eventToEdit.authorId;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (eventToEdit?.recurringEventId) {
      setRecurringModalState({ isOpen: true, event: eventData as Event, action: 'update' });
      return;
    }
    ...
    if(eventToEdit) {
        ...
        addEvent(eventData as Omit<Event, 'id' | 'authorId'>);
  ...
  const handleDeleteClick = () => {
    if (eventToEdit && canEdit) {
      ...
      deleteEvent(eventToEdit.id, eventToEdit.title);
      onClose();
    }
  };
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col animate-slideIn">
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0">
              ...
              <div className="px-6 pb-6 border-t border-slate-200 dark:border-slate-700 mt-4">
                ...
                <div className="space-y-4 max-h-48 overflow-y-auto pr-2">
                  {eventComments.map(comment => {
                      const commentAuthor = users.find(u => u.uid === comment.authorId);
                      const canDeleteComment = currentUser.role === 'admin' || currentUser.uid === comment.authorId;
                      return (
                        <div key={comment.id} className="flex items-start gap-3 group">
                          ...
                          {canDeleteComment && (
                              <button type="button" onClick={() => handleDeleteCommentClick(comment.id)} className="p-1 rounded-full text-slate-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <div className="flex-shrink-0 flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-700">
            <div>
              {eventToEdit && canEdit && (
                <button type="button" onClick={handleDeleteClick} className="px-4 py-2 text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-md">{t('event.delete')}</button>
              )}
            </div>
