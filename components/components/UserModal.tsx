...
const UserModal: React.FC<UserModalProps> = ({ userToEdit, onClose }) => {
  const { updateUser, addToast } = useAppContext();
  ...
  const getInitialState = (): User => {
    ...
    // "Add User" is handled by signup flow, so this modal is for editing only.
    // This is a fallback, but shouldn't be reached in normal flow.
    return {
      id: '',
      uid: '',
      name: '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formState.id) {
      ...
      addToast({message: "Cannot create user from this modal.", type: 'error'});
    }
    onClose();
  };
